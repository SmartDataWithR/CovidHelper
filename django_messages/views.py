...
@login_required
def compose_with_userto(request, user_to, recipient=None, form_class=ComposeForm,
        template_name='django_messages/compose.html', success_url=None,
        recipient_filter=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # send the mail
            try:
                send_mail(subject, body, 'info@dogooddeed.com', [email_recipient])
...
def overview(request):
    return render(request, 'django_messages/base.html')