...
def index(request):
    ...
    if search != None:
        ...
        #     print(u.longitude)
        df = pd.DataFrame([u.group_membership, u.longitude, u.latitude, u.slogan, u.description] for u in CustomUser.objects.raw('SELECT * FROM users_customuser') )
        
        df.columns = ['group_membership', 'longitude', 'latitude', 'slogan', 'description'] # 
        ...
        context = {'longitude': location.longitude, 'latitude': location.latitude, 'group_membership': group_membership, 'longitudes': longitudes, 'latitudes': latitudes, 'slogan': slogan, 'description': description}
